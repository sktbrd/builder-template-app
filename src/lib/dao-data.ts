...
export async function getDashboardData(): Promise<DashboardData> {
  ...
  const [recentProposerEns, voterEnsMap, treasuryBalances, currentAuctionBids] =
    await Promise.all([
      ...
      // Recent bids (with comments) come straight from the subgraph — it decodes
      // the calldata-trailing-bytes comment server-side and exposes it as
      // `comment`, so no on-chain getLogs + per-tx reads are needed here. Past
      // tokens collapse to just their winning bid, so we never need their history.
      currentAuction
        ? safeFetch(
  ...
  if (currentAuction) {
    // getBids() returns newest-first, ETH-formatted amounts; keep the latest 5.
